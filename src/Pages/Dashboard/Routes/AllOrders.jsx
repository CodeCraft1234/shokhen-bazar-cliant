...
const AllOrders = () => { 
    ...
    const handleCancelOrder = async (orderId) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await AxiosPublic.delete(`http://localhost:5000/orders/${orderId}`);
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        try {
            const response = await AxiosPublic.patch(`http://localhost:5000/orders/${orderId}`, { status: newStatus });
            console.log(response.data);
            refetch();

...
import { FaCartPlus, FaEye, FaHeart, FaPhoneAlt, FaRegUserCircle } from "react-icons/fa";
import { Helmet } from "react-helmet-async";
const ProductDetails = () => {
    const ProductDetails=useLoaderData()
  
    const { image,discount,subImages, price,brand, short, category, title, _id,originalPrice } = ProductDetails;
...
useEffect(() => {
  ...
  const handleSubmitted = (e) => {
    ...
    console.log(body)
    fetch("https://hirikbazar.vercel.app/feedbacks", {
      method: "POST",
      headers: {
...
const navigate = useNavigate();
  ...
  const ratings = [1, 2, 3, 4, 5].map(stars => ({
    ...
    return (
            <div className="pt-20 text-black">
              <Helmet>
              <title> Shokher Bazar | ProductDetails</title>
  ...
  <div className="grid grid-cols-4 md:grid-cols-4 order-2 lg:order-1 md:order-2 sm:grid-cols-2 lg:grid-cols-1 lg:col-span-1 ">
  ...
  <div className="lg:col-span-3 order-1 lg:order-2 md:order-1 sm:col-span-2 items-center">
   ...
   <div className="lg:col-span-3 order-3 lg:order-3 md:order-3 sm:col-span-2">
      <div className="p-4">
        ...
        <div className="py-3">
          <button className="bg-[#FF7701] text-white font-bold py-2 px-4 rounded-md shadow-md w-full">
            <FaPhoneAlt className="inline-block mr-2" /> কল করতে ক্লিক করুন
          </button>
        </div>
      ...
      <h1 className="text-base text-red-400 font-bold">{feedback.textarea}</h1>
        ...
        <div className="mx-4 py-3">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 md:grid-cols-4 gap-3">
              {related.map((item) => (
                <div key={item._id} className="border rounded-lg overflow-hidden shadow-lg flex flex-col justify-between lg:h-full relative">
                <Link to={`/productDetails/${item?._id}`} className="flex flex-col h-full">
                  <img
                    className="w-full h-36 lg:h-56 md:h-44 transform transition-transform duration-500 hover:scale-110"
                    src={item.image}
                    alt=''
                  />
                  {item.discount > 0 && (
        ...
        <h2 className=" text-base hover:text-blue-700 text-black hover:font-bold lg:text-lg">{item.title}</h2>
        <p className="text-red-300 text-base lg:text-lg flex justify-between font-medium">
          <span className="text-red-600 hover:text-blue-600 ">{item.price}</span>{" "}
          {item.discount > 0 && (
